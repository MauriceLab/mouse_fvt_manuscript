...
fvt.host.targets.species <- ps.phage.relabund %>% 
  ...
  subset_samples(SampleName == "VIVO7-DONOR2") %>% 
  psmelt() %>% 
...
bact.hosts.present <- ps.relabund %>% 
  ...
  subset_samples(AnimalID != "POOL-F11") %>% 
  subset_samples(AnimalID != "POOL-F2") %>% 
  psmelt() %>% 
...
host.presence.results %>% 
  ...
  #select(SampleName, AnimalID, Dam, PostnatalDay, prop_species_targets_present, prop_genus_targets_present)
...
# genera in pups
bact.hosts.present$genus_list %>% unlist() %>% unique() %>% dput()

#genera in FVT
fvt.host.targets.species$genus %>% unique() %>% dput()
