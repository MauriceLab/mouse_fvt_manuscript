...
ps.phage.relabund.otutable <- ps.phage.relabund %>% 
  subset_samples(Group %in% c(1)) %>% # repeat with group set to 1, 2
  otu_table() %>% 
  as.data.frame()
...
fit_out <- maaslin3(input_data = ps.phage.relabund.otutable,
                    ...
                    output = 'maaslin3-out-vlp-group1',
...
maaslin.results.g1 <- read_tsv("analysis-exp2/maaslin3-out-vlp-group1/all_results.tsv")
maaslin.results.g2 <- read_tsv("analysis-exp2/maaslin3-out-vlp-group2/all_results.tsv")
...
maaslin.results.g1.signif <- maaslin.results.g1 %>% 
  filter(model == "abundance") %>% 
  filter(name == "PrePostFVTPost") %>% 
  mutate(signif = ifelse((feature %!in% maaslin.results.g2.signif) & qval_individual <= 0.1, T, F)) %>% 
...
set.seed(1234)
fig.3c2 <- maaslin.results.g1.signif %>% 
  ...
  ggtitle("Virome - Experiment #2")


fig.3c2

fig.s3b <- ps.phage.relabund %>% 
  ...
  facet_wrap(~ OTU, nrow = 3, scales="free_y") +
